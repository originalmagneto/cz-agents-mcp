...
export function buildEuRegistryServer(options: EuRegistryServerOptions = {}): McpServer {
  ...
  server.tool(
    'search_company',
    'Search non-Czech business registries by company name. Free tier: GB (Companies House), SK (RPO), PL (KRS), DE (GLEIF/LEI). Compliance tier: FR (SIRENE).',
    {
  ...
  server.tool(
    'get_company',
    'Get a non-Czech company by national ID and country code. Supported: gb (CRN), sk (IČO), pl (KRS number), de (LEI), fr (SIREN).',
    {
